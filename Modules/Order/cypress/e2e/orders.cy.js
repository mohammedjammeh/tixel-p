describe("Orders tests", () => {
    ...
    it("makes request to orders index and displays them on page", () => {
        cy.getDataTest("order_name").should("have.text", "Order #1");

        cy.getDataTest("meals_names").should("have.text", "cheese, tomato");
    });
    ...
    it("displays solid check for order status and those behind, a loader for to next status and regular check for front statuses", () => {
        ...
        cy.getDataTest("order_1_new_loader").should("not.exist");
        ...
        cy.getDataTest("order_1_prepared_loader").should("not.exist");
        ...
        // cooked (next status)
        cy.getDataTest("order_1_cooked_loader").should("exist").should("be.visible");

        cy.getDataTest("order_1_cooked_check").should("not.exist");

        // ready (front status)
        cy.getDataTest("order_1_ready_loader").should("not.exist");
    ...
    it("adds text-blue-900 class to order status elements and those behind, text-gray-200 for to next and front statuses", () => {
        ...
        // cooked (next status)
        cy.getDataTest("order_1_cooked_status")
    ...
    it("updates displayed order elements after updating order status", () => {
        ...
        cy.getDataTest("order_1_cooked_loader").should("not.exist");
